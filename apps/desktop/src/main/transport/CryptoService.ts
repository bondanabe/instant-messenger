...
function toBlob(data: unknown): Uint8Array | null {
  if (!data) return null
...
// ─────────────────────────────────────────────────────────────────
// DESKTOP CRYPTO SERVICE
// Sama dengan mobile, tapi DB query via better-sqlite3 (synchronous di
// belakang drizzle — namun drizzle tetap return Promise)
// ─────────────────────────────────────────────────────────────────

export class DesktopCryptoService {
  ...
  async getOurPreKeyBundle(): Promise<PreKeyBundleWire> {
    ...
    if (!spkRows[0]) throw new Error('DesktopCryptoService: tidak ada signed prekey tersedia')
    const spk = spkRows[0]
    ...
    const bundle: PreKeyBundleWire = {
      ...
      signedPreKey: b64(toBlob(spk.publicKey)!),
      signedPreKeySignature: b64(toBlob(spk.signature)!),
    }
    if (opk) {
      bundle.oneTimePreKeyId = opk.id
      bundle.oneTimePreKey = b64(toBlob(opk.publicKey)!)
    }
    return bundle
  }
  ...
  async encryptFor(targetUserId: string, wireMessage: WireMessage): Promise<E2EEPayload> {
    ...
    if (!session) {
      ...
      if (initHeader.oneTimePreKeyId !== undefined) {
        x3dhHeader.oneTimePreKeyId = initHeader.oneTimePreKeyId
      }
    ...
    if (x3dhHeader) result.x3dh = x3dhHeader
    return result
  }
  ...
  async decrypt(fromUserId: string, payload: E2EEPayload): Promise<WireMessage> {
    ...
    if (!session && payload.x3dh) {
      ...
      if (!spkRows[0]) {
        throw new Error(`DesktopCryptoService: signed prekey #${x3dhHdr.signedPreKeyId} tidak ditemukan`)
      }

      const spkPair: DHKeyPair = {
        privateKey: toBlob(spkRows[0].privateKey)!,
        publicKey: toBlob(spkRows[0].publicKey)!,
      }
      ...
      if (x3dhHdr.oneTimePreKeyId !== undefined) {
        ...
        if (opkRows[0]) {
          opkPair = {
            privateKey: toBlob(opkRows[0].privateKey)!,
            publicKey: toBlob(opkRows[0].publicKey)!,
          }
          await db
            .update(prekeysTable)
            .set({ usedAt: Date.now() })
      ...
      const identity = await this.loadIdentity()
      const respondParams: Parameters<typeof x3dhRespond>[0] = {
        ourIdentityKeyPair: identity,
        ourSignedPreKeyPair: spkPair,
        initHeader: x3dhHdr,
      }
      if (opkPair) respondParams.ourOneTimePreKeyPair = opkPair
      const sessionKey = x3dhRespond(respondParams)

      session = initRatchetAsReceiver(sessionKey, spkPair)
    }

    if (!session) {
      throw new Error(`DesktopCryptoService: tidak ada sesi untuk ${fromUserId} dan tidak ada X3DH header`)
    }
  ...
  private wireToBundle(wire: PreKeyBundleWire): PreKeyBundle {
    ...
    if (wire.oneTimePreKeyId !== undefined) base.oneTimePreKeyId = wire.oneTimePreKeyId
    if (wire.oneTimePreKey) base.oneTimePreKey = fromb64(wire.oneTimePreKey)
    return base
  }
  ...
  private async loadIdentity(): Promise<IdentityKeyPair> {
    ...
    if (!rows[0]) throw new Error('DesktopCryptoService: identitas tidak ditemukan di database')
    this.identityCache = {
      privateKey: toBlob(rows[0].privateKey)!,
      publicKey: toBlob(rows[0].publicKey)!,
    }
  ...
  private async loadSession(userId: string): Promise<RatchetState | null> {
    ...
    return deserializeRatchetState(toBlob(rows[0].sessionData)!)
  }
